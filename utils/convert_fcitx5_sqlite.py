...
for x in file_list:
    ...
    tables[schema]['MetroboardLocale'] = f"{conf['InputMethod']['LangCode']}_{schema}"
...
# Writing
language_pack = [dict(id=table['MetroboardLocale'], hanShapeBasedKeyCode=table['KeyCode']) for schema, table in tables.items()]
...
for schema, table in tables.items():
    ...
    # put_table(database, table['MetroboardLocale'], table)
