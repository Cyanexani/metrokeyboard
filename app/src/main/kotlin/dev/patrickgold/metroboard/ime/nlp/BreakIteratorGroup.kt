...
open class BreakIteratorGroup {
    private val charInstances = Cache.Builder<MetroboardLocale, GuardedByLock<BreakIterator>>().build()

    private val wordInstances = Cache.Builder<MetroboardLocale, GuardedByLock<BreakIterator>>().build()

    private val sentenceInstances = Cache.Builder<MetroboardLocale, GuardedByLock<BreakIterator>>().build()

    suspend fun <R> character(locale: MetroboardLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun <R> word(locale: MetroboardLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun <R> sentence(locale: MetroboardLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun measureUChars(
        text: String,
        numUnicodeChars: Int,
        locale: MetroboardLocale = MetroboardLocale.default(),
    ): Int {
