...
import org.metroboard.lib.compose.toDp
...
private fun ImeWindowResizeHandle(
    ...
    val style = rememberSnyggThemeQuery(MetroboardImeUi.WindowResizeHandle.elementName, attributes)
...
fun BoxScope.ImeWindowResizeHandlesFixed() {
    ...
    val attributes = remember(windowConfig.mode) {
        mapOf(
            MetroboardImeUi.Attr.WindowMode to windowConfig.mode
        )
    }

    val overlayStyle = rememberSnyggThemeQuery(MetroboardImeUi.WindowResizeOverlayFixed.elementName)
    ...
    if (visible) {
        ...
        Row(
            ...
            SnyggIconButton(
                elementName = MetroboardImeUi.WindowResizeAction.elementName,
            ...
            SnyggIconButton(
                MetroboardImeUi.WindowMoveHandle.elementName,
                attributes = attributes,
                onClick = {}
            ) {
                SnyggIcon(
                    elementName = MetroboardImeUi.WindowMoveHandle.elementName,
...
private fun Modifier.imeWindowEditorHandle(
    ...
    val rowCount by MetroboardImeSizing.rowCountAsState()
    val smartbarRowCount by MetroboardImeSizing.smartbarRowCountAsState()
    Modifier
        .pointerInput(Unit) {
            detectTapGestures {
                onTap()
