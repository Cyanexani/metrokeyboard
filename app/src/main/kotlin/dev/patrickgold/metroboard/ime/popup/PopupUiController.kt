...
package dev.patrickgold.metroboard.ime.popup
...
import dev.patrickgold.metroboard.ime.media.emoji.EmojiSet
...
import dev.patrickgold.metroboard.ime.text.keyboard.TextKey
...
fun rememberPopupUiController(
    key1: Any?,
    key2: Any?,
    boundsProvider: (key: Key) -> MetroboardRect,
...
class PopupUiController(
    val context: Context,
    val boundsProvider: (key: Key) -> MetroboardRect,
    ...
    fun extend(key: Key, size: Size) {
        ...
        val extHeight = baseBounds.height * 0.9f
        ...
        val y = -baseBounds.height + key.visibleBounds.bottom
        val extBounds = MetroboardRect.new(
    ...
    fun RenderPopups(): Unit = with(LocalDensity.current) {
        ...
        extRenderInfo?.let { renderInfo ->
            ...
            val elemHeight = baseBounds.height * 0.9f
            PopupExtBox(
                modifier = Modifier
                    .requiredSize(renderInfo.bounds.size.toDpSize())
                    .absoluteOffset { renderInfo.bounds.topLeft.toIntOffset() },
