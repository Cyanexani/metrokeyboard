...
package dev.patrickgold.metroboard.ime.keyboard
...
import dev.patrickgold.metroboard.appContext
...
import dev.patrickgold.metroboard.ime.core.SubtypePreset
import dev.patrickgold.metroboard.ime.editor.EditorContent
import dev.patrickgold.metroboard.ime.editor.MetroboardEditorInfo
import dev.patrickgold.metroboard.ime.editor.ImeOptions
import dev.patrickgold.metroboard.ime.editor.InputAttributes
...
import dev.patrickgold.metroboard.ime.input.InputKeyEventReceiver
import dev.patrickgold.metroboard.ime.input.InputShiftState
import dev.patrickgold.metroboard.ime.nlp.ClipboardSuggestionCandidate
import dev.patrickgold.metroboard.ime.nlp.PunctuationRule
import dev.patrickgold.metroboard.ime.nlp.SuggestionCandidate
import dev.patrickgold.metroboard.ime.popup.PopupMappingComponent
import dev.patrickgold.metroboard.ime.text.composing.Composer
...
import dev.patrickgold.metroboard.ime.text.keyboard.TextKeyboardCache
...
import dev.patrickgold.metroboard.lib.titlecase
import dev.patrickgold.metroboard.lib.uppercase
import dev.patrickgold.metroboard.lib.util.InputMethodUtils
import dev.patrickgold.metroboard.nlpManager
...
import org.metroboard.lib.android.showLongToast
...
class KeyboardManager(context: Context) : InputKeyEventReceiver {
     ...
     * MetroboardBoard internal or system-wide.
     */
    private fun handleLanguageSwitch() {
        when (prefs.keyboard.utilityKeyAction.get()) {
            ...
            else -> MetroboardImeService.switchToNextInputMethod()
        }
    }
        ...
        lastToastReference = WeakReference(
            if (newState) {
                appContext.showLongToast(
                    ...
                    "app_name" to appContext.getString(R.string.metroboard_app_name),
                )
            } else {
                appContext.showLongToast(
    ...
    override fun onInputKeyDown(data: KeyData) {
        val windowController = MetroboardImeService.windowControllerOrNull()
    ...
    override fun onInputKeyUp(data: KeyData) = activeState.batchEdit {
        val windowController = MetroboardImeService.windowControllerOrNull() ?: return@batchEdit
        when (data.code) {
            KeyCode.ARROW_DOWN,
            KeyCode.ARROW_LEFT,
            KeyCode.ARROW_RIGHT,
            ...
            KeyCode.IME_SHOW_UI -> MetroboardImeService.showUi()
            KeyCode.IME_HIDE_UI -> MetroboardImeService.hideUi()
            ...
            KeyCode.VOICE_INPUT -> MetroboardImeService.switchToVoiceInputMethod()
            ...
            KeyCode.SETTINGS -> MetroboardImeService.launchSettings()
            ...
            KeyCode.SYSTEM_PREV_INPUT_METHOD -> MetroboardImeService.switchToPrevInputMethod()
            KeyCode.SYSTEM_NEXT_INPUT_METHOD -> MetroboardImeService.switchToNextInputMethod()
    ...
    override fun onInputKeyRepeat(data: KeyData) {
        MetroboardImeService.inputFeedbackController()?.keyRepeatedAction(data)
        when (data.code) {
            KeyCode.ARROW_DOWN,
            KeyCode.ARROW_LEFT,
            KeyCode.ARROW_RIGHT,
    ...
    private inner class ComputingEvaluatorImpl(
        ...
        override val editorInfo: MetroboardEditorInfo,
