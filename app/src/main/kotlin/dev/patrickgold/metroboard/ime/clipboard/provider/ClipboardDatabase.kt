...
package dev.patrickgold.metroboard.ime.clipboard.provider
...
import org.metroboard.lib.android.UriSerializer
...
data class ClipboardItem @OptIn(ExperimentalSerializationApi::class) constructor(
    ...
    companion object {
        ...
        const val METROBOARD_CLIP_LABEL = "metroboard/clipboard_item"
    ...
    fun toClipData(context: Context): ClipData {
        return when (type) {
            ItemType.TEXT -> {
                ClipData.newPlainText(METROBOARD_CLIP_LABEL, text)
            }
            ItemType.IMAGE, ItemType.VIDEO -> {
                ClipData.newUri(context.contentResolver, METROBOARD_CLIP_LABEL, uri)
            }
        }
    }
