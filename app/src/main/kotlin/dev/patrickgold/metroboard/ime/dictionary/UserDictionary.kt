...
import dev.patrickgold.metroboard.lib.ValidationRule
import org.metroboard.lib.android.readText
import org.metroboard.lib.android.writeText
...
interface UserDictionaryDao {
    ...
    fun query(word: String, locale: MetroboardLocale?): List<UserDictionaryEntry>
    ...
    fun queryShortcut(shortcut: String, locale: MetroboardLocale?): List<UserDictionaryEntry>
    ...
    fun queryAll(locale: MetroboardLocale?): List<UserDictionaryEntry>
    ...
    fun queryExact(word: String, locale: MetroboardLocale?): List<UserDictionaryEntry>

    @Query("$SELECT_ALL_FROM_WORDS WHERE ${UserDictionary.Words.WORD} = :word AND $LOCALE_MATCHES")
    fun queryExactFuzzyLocale(word: String, locale: MetroboardLocale?): List<UserDictionaryEntry>

    @Query("SELECT DISTINCT ${UserDictionary.Words.LOCALE} FROM $WORDS_TABLE")
    fun queryLanguageList(): List<MetroboardLocale?>
            ...
            src.forEachLine { line ->
                if (isFirstLine) {
                    ...
                    val alreadyExistingEntries = userDictionaryDao().queryExact(
                        word, locale?.let { MetroboardLocale.fromTag(it) },
                    )
...
@TypeConverters(MetroboardUserDictionaryDatabase.Converters::class)
abstract class MetroboardUserDictionaryDatabase : RoomDatabase(), UserDictionaryDatabase {
    companion object {
        const val DB_FILE_NAME = "metroboard_user_dictionary"
    }
    ...
    class Converters {
        @TypeConverter
        fun localeToString(locale: MetroboardLocale?): String? {
            return when (locale) {
                null -> null
        ...
        @TypeConverter
        fun stringToLocale(string: String?): MetroboardLocale? {
            return when (string) {
                null, "all", "null", "" -> null
                else -> MetroboardLocale.fromTag(string)
            }
        }
    }
}
...
class SystemUserDictionaryDatabase(context: Context) : UserDictionaryDatabase {
    ...
    private val dao = object : UserDictionaryDao {
        ...
        override fun query(word: String, locale: MetroboardLocale?): List<UserDictionaryEntry> {
        ...
        override fun queryShortcut(shortcut: String, locale: MetroboardLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryAll(locale: MetroboardLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryExact(word: String, locale: MetroboardLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryExactFuzzyLocale(word: String, locale: MetroboardLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
                    selection = "${UserDictionary.Words.WORD} = ? AND ${UserDictionary.Words.LOCALE} IS NULL",
                    selectionArgs = arrayOf(word),
        ...
        override fun queryLanguageList(): List<MetroboardLocale?> {
            ...
            val retList = mutableSetOf<MetroboardLocale?>()
            while (cursor.moveToNext()) {
                val localeStr = cursor.getString(localeIndex)
                if (localeStr == null) {
                    retList.add(null)
                } else {
                    retList.add(MetroboardLocale.fromTag(localeStr))
                }
            }
            cursor.close()
...
object UserDictionaryValidation {
    ...
    val Locale = ValidationRule<String> {
        ...
        validator { input ->
            ...
            when {
                input.isBlank() -> resultValid() // Is optional
                tryOrNull { MetroboardLocale.fromTag(str) } == null -> resultInvalid(error = R.string.settings__udm__dialog__locale_error_invalid)
                else -> resultValid()
            }
        }
    }
