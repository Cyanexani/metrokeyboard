...
import org.metroboard.lib.android.copyRecursively
import org.metroboard.lib.android.write
...
object ZipUtils {
    fun readFileFromArchive(context: Context, zipRef: MetroboardRef, relPath: String) = runCatching<String> {
        when {
            zipRef.isAssets -> {
    ...
    fun zip(context: Context, srcRef: MetroboardRef, dstRef: MetroboardRef) =
        zip(context, FsDir(srcRef.absolutePath(context)), dstRef)

    fun zip(context: Context, srcDir: FsDir, dstRef: MetroboardRef) = runCatching {
    ...
    fun unzip(context: Context, srcRef: MetroboardRef, dstRef: MetroboardRef) =
        unzip(context, srcRef, FsDir(dstRef.absolutePath(context)))

    fun unzip(context: Context, srcRef: MetroboardRef, dstDir: FsFile) = runCatching {
