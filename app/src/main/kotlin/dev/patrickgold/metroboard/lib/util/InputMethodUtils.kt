...
package dev.patrickgold.metroboard.lib.util
...
import org.metroboard.lib.android.AndroidSettings
import org.metroboard.lib.android.AndroidVersion
import org.metroboard.lib.android.systemServiceOrNull
import org.metroboard.lib.compose.observeAsState

private const val DELIMITER = ':'
private const val IME_SERVICE_CLASS_NAME = "dev.patrickgold.metroboard.MetroboardImeService"
private const val TIMED_QUERY_DELAY = 500L

object InputMethodUtils {
    fun isMetroboardEnabled(context: Context): Boolean {
        return if (AndroidVersion.ATLEAST_API34_U) {
            ...
            enabledImeList != null && parseIsMetroboardEnabled(context, enabledImeList)
        }
    }

    fun isMetroboardSelected(context: Context): Boolean {
        return if (AndroidVersion.ATLEAST_API34_U) {
            context.systemServiceOrNull(InputMethodManager::class)
            ...
            selectedIme != null && parseIsMetroboardSelected(context, selectedIme)
        }
    }

    @Composable
    fun observeIsMetroboardEnabled(
        context: Context = LocalContext.current.applicationContext,
        foregroundOnly: Boolean = false,
    ): State<Boolean> {
        return if (AndroidVersion.ATLEAST_API34_U) {
            timedObserveIsMetroboardBoardEnabled()
        } else {
            AndroidSettings.Secure.observeAsState(
                key = Settings.Secure.ENABLED_INPUT_METHODS,
                foregroundOnly = foregroundOnly,
                transform = { parseIsMetroboardEnabled(context, it.toString()) },
            )
        }
    }

    @Composable
    fun observeIsMetroboardSelected(
        context: Context = LocalContext.current.applicationContext,
        foregroundOnly: Boolean = false,
    ): State<Boolean> {
        return if (AndroidVersion.ATLEAST_API34_U) {
            timedObserveIsMetroboardBoardSelected()
        } else {
            AndroidSettings.Secure.observeAsState(
                key = Settings.Secure.DEFAULT_INPUT_METHOD,
                foregroundOnly = foregroundOnly,
                transform = { parseIsMetroboardSelected(context, it.toString()) },
            )
        }
    }

    fun parseIsMetroboardEnabled(context: Context, activeImeIds: String): Boolean {
    ...
    fun parseIsMetroboardSelected(context: Context, selectedImeId: String): Boolean {
    ...
    @RequiresApi(api = 34)
    @Composable
    private fun timedObserveIsMetroboardBoardEnabled(): State<Boolean> {
        val state = remember { mutableStateOf(false) }
        val context = LocalContext.current
        LaunchedEffect(Unit) {
            while (true) {
                state.value = isMetroboardEnabled(context)
    ...
    @RequiresApi(api = 34)
    @Composable
    private fun timedObserveIsMetroboardBoardSelected(): State<Boolean> {
        val state = remember { mutableStateOf(false) }
        val context = LocalContext.current
        LaunchedEffect(Unit) {
            while (true) {
                state.value = isMetroboardSelected(context)
                delay(TIMED_QUERY_DELAY)
            }
        }
        return state
