...
package dev.patrickgold.metroboard.lib.crashutility
...
import dev.patrickgold.metroboard.lib.devtools.Devtools
...
private class SafePreferenceInstanceWrapper : ReadOnlyProperty<Any?, MetroboardPreferenceModel?> {
    ...
    override fun getValue(thisRef: Any?, property: KProperty<*>): MetroboardPreferenceModel? {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val versionName = buildString {
            ...
            if (BuildConfig.DEBUG) {
                append(stringRes(R.string.metroboard__commit_by_hash_url, "hash" to BuildConfig.BUILD_COMMIT_HASH))
            } else {
                append(stringRes(R.string.metroboard__changelog_url, "version" to BuildConfig.VERSION_NAME))
            }
            append(")")
        }
        errorReport.apply {
            appendLine("#### Environment information")
            appendLine("- MetroboardBoard $versionName (${BuildConfig.VERSION_CODE})")
        ...
        openBugReportForm.setOnClickListener {
            val browserIntent = Intent(
                Intent.ACTION_VIEW,
                Uri.parse(resources.getString(R.string.metroboard__issue_tracker_url))
            )
