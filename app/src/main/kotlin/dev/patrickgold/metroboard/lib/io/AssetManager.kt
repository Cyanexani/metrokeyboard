/*
 * Copyright (C) 2020-2025 The MetroboardBoard Contributors
 *
...
import dev.patrickgold.metroboard.ime.keyboard.AbstractKeyData
import dev.patrickgold.metroboard.ime.keyboard.CaseSelector
import dev.patrickgold.metroboard.ime.keyboard.CharWidthSelector
import dev.patrickgold.metroboard.ime.keyboard.KanaSelector
import dev.patrickgold.metroboard.ime.keyboard.KeyData
import dev.patrickgold.metroboard.ime.keyboard.LayoutDirectionSelector
import dev.patrickgold.metroboard.ime.keyboard.ShiftStateSelector
import dev.patrickgold.metroboard.ime.keyboard.VariationSelector
import dev.patrickgold.metroboard.ime.text.keyboard.AutoTextKeyData
import dev.patrickgold.metroboard.ime.text.keyboard.MultiTextKeyData
...
import org.metroboard.lib.android.reader
import org.metroboard.lib.kotlin.resultErr
import org.metroboard.lib.kotlin.resultErrStr
import org.metroboard.lib.kotlin.resultOk
...
fun MetroboardRef.delete(context: Context) {
    when {
        isCache || isInternal -> {
...
fun MetroboardRef.hasAsset(context: Context): Boolean {
    return when {
        isAssets -> {
            try {
...
fun MetroboardRef.list(context: Context) = list(context, files = true, dirs = true)

fun MetroboardRef.listFiles(context: Context) = list(context, files = true, dirs = false)

fun MetroboardRef.listDirs(context: Context) = list(context, files = false, dirs = true)

private fun MetroboardRef.list(appContext: Context, files: Boolean, dirs: Boolean) = runCatching<List<MetroboardRef>> {
    when {
        !files && !dirs -> listOf()
        isAssets -> {
        ...
        else -> error("Unsupported MetroboardRef source!")
    }
}

fun <T> MetroboardRef.loadJsonAsset(
    context: Context,
...
fun MetroboardRef.loadTextAsset(context: Context): Result<String> {
    return when {
