...
package dev.patrickgold.metroboard.lib

import android.content.Context
import dev.patrickgold.metroboard.extensionManager
import dev.patrickgold.metroboard.lib.MetroboardLocale.Companion.default
import java.util.*
 ...
 *
 * To construct a MetroboardLocale, use one of the many from() methods provided.
 *
 * @see java.util.Locale
 */
@Serializable(with = MetroboardLocale.Serializer::class)
class MetroboardLocale private constructor(val base: Locale) {
    companion object {
        ...
        /**
         * Wraps a [java.util.Locale] and returns the [MetroboardLocale].
         *
         * @return The wrapped locale.
         */
        fun from(javaLocale: Locale) = MetroboardLocale(javaLocale)

        /**
         * Constructs a new [MetroboardLocale] with given [language].
         *
         * @param language A two-letter language code.
         *
         * @return A new [MetroboardLocale].
         */
        fun from(language: String) = from(Locale(language))

        /**
         * Constructs a new [MetroboardLocale] with given [language] and [country].
         *
         * @param language A two-letter language code.
        ...
        /**
         * Constructs a new [MetroboardLocale] with given [language], [country] and [variant].
         *
         * @param language A two-letter language code.
         * @param country A two-letter country code.
        ...
        /**
         * Constructs a new [MetroboardLocale] from given [str].
         *
         ...
         */
        fun default() = MetroboardLocale(Locale.getDefault())
         ...
         */
        fun installedSystemLocales(): List<MetroboardLocale> = Locale.getAvailableLocales().map { from(it) }

        /**
         * Returns a list of all installed locales and custom locales.
         *
         */
        fun extendedAvailableLocales(context: Context): List<MetroboardLocale> {
     ...
     */
    fun displayLanguage(locale: MetroboardLocale = default()): String {
     ...
     */
    fun displayCountry(locale: MetroboardLocale = default()): String = base.getDisplayCountry(locale.base)
     ...
     */
    fun displayVariant(locale: MetroboardLocale = default()): String = base.getDisplayVariant(locale.base)
     ...
     *  [language], [country] and [variant] are not specified.
     */
    fun displayName(locale: MetroboardLocale = default()) = buildString {
     ...
     */
    override fun toString() = "MetroboardLocale { l=${base.language} c=${base.country} v=${base.variant} }"
    ...
    /**
     * The JSON (de)serializer for MetroboardLocale.
     */
    class Serializer : KSerializer<MetroboardLocale> {
        override val descriptor: SerialDescriptor =
            PrimitiveSerialDescriptor("MetroboardLocale", PrimitiveKind.STRING)

        override fun serialize(encoder: Encoder, value: MetroboardLocale) {
            encoder.encodeString(value.languageTag())
        }

        override fun deserialize(decoder: Decoder): MetroboardLocale {
...
inline fun String.lowercase(locale: MetroboardLocale): String = this.lowercase(locale.base)

@Suppress("NOTHING_TO_INLINE")
inline fun String.uppercase(locale: MetroboardLocale): String = this.uppercase(locale.base)

@Suppress("NOTHING_TO_INLINE")
inline fun String.titlecase(locale: MetroboardLocale = MetroboardLocale.ROOT): String {
