...
import dev.patrickgold.metroboard.ime.keyboard.DefaultComputingEvaluator
import dev.patrickgold.metroboard.ime.keyboard.Key
import dev.patrickgold.metroboard.ime.keyboard.KeyData
import dev.patrickgold.metroboard.ime.keyboard.Keyboard
import dev.patrickgold.metroboard.ime.keyboard.KeyboardMode
import dev.patrickgold.metroboard.ime.keyboard.computeImageVector
import dev.patrickgold.metroboard.ime.keyboard.computeLabel
...
import dev.patrickgold.metroboard.ime.window.ImeWindowMode
import dev.patrickgold.metroboard.keyboardManager
import dev.patrickgold.metroboard.lib.NATIVE_NULLPTR
...
import org.metroboard.lib.snygg.SnyggAttributes
...
import org.metroboard.lib.snygg.ui.NonNullSaver
...
internal fun EditRuleDialog(
    ...
    val possibleRuleTemplates = remember {
        buildList {
            ...
            MetroboardImeUi.elementNames.forEach { name ->
        ...
        Column {
            ...
            (currentRule as? SnyggElementRule)?.apply {
                ...
                DialogProperty(text = stringRes(R.string.settings__theme_editor__rule_selectors)) {
                    Row(
                        modifier = Modifier.metroboardHorizontalScroll(),
                ...
                val codes = remember(currentRule) {
                    attributes[MetroboardImeUi.Attr.Code] ?: emptyList()
                }
                ...
                if (initCodeValue != null) {
                    EditCodeValueDialog(
                        ...
                        onAdd = {
                            currentRule = copy(
                                attributes = attributes.including(MetroboardImeUi.Attr.Code to it)
                            )
                        },
                        onDelete = {
                            currentRule = copy(
                                attributes = attributes.excluding(MetroboardImeUi.Attr.Code to it)
                            )
                        },
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = MetroboardImeUi.Attr.Mode,
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = MetroboardImeUi.Attr.ShiftState,
                    attributes = attributes,
                    setAttributes = { currentRule = copy(attributes = it) },
                    level = level,
                )
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = MetroboardImeUi.Attr.WindowMode,
                    attributes = attributes,
                    setAttributes = { currentRule = copy(attributes = it) },
                    level = level,
                )
...
private fun EditCodeValueDialog(
    ...
    fun requestStartRecording() {
        ...
        if (!isMetroboardBoardEnabled || !isMetroboardBoardSelected) {
            lastRecordingToast?.cancel()
            lastRecordingToast = context.showShortToastSync(
                R.string.settings__theme_editor__code_recording_requires_default_ime_metroboard,
                "app_name" to context.stringRes(R.string.metroboard_app_name),
            )
            ...
            AnimatedVisibility(visible = showKeyCodesHelp) {
                Column(modifier = Modifier.padding(bottom = 16.dp)) {
                    ...
                    MetroboardHyperlinkText(
                        ...
                        url = stringRes(R.string.metroboard__character_key_codes_url),
                    )
                    MetroboardHyperlinkText(
                        text = "Internal (github.com)",
                        url = stringRes(R.string.metroboard__internal_key_codes_url),
                    )
                }
            }
