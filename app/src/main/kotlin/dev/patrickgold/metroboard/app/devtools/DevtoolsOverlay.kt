...
package dev.patrickgold.metroboard.app.devtools
...
import dev.patrickgold.metroboard.ime.keyboard.CachedLayout
import dev.patrickgold.metroboard.ime.keyboard.DebugLayoutComputationResult
...
import org.metroboard.lib.snygg.SnyggMissingSchemaException

private val CardBackground = Color.Black.copy(0.6f)
private val DateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH-mm-ss", MetroboardLocale.default().base)
    ...
    DevtoolsOverlayBox(title = "Failed to load stylesheet, fell back to base style") {
        ...
        if (cause is SnyggMissingSchemaException) {
            DevtoolsSubGroup(title = "Explanation") {
                DevtoolsText(
                    text = """
                    It appears you’re trying to load a theme designed for MetroboardBoard v0.4 (Snygg v1), which isn’t compatible with the latest release using Snygg v2.
