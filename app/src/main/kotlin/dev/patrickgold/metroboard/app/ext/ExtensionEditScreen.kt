...
import dev.patrickgold.metroboard.app.settings.theme.DialogProperty
import dev.patrickgold.metroboard.app.settings.theme.PrettyPrintConfig
import dev.patrickgold.metroboard.app.settings.theme.ThemeEditorScreen
import dev.patrickgold.metroboard.cacheManager
import dev.patrickgold.metroboard.extensionManager
import dev.patrickgold.metroboard.ime.keyboard.KeyboardExtension
...
import dev.patrickgold.metroboard.ime.theme.ThemeExtensionComponentImpl
...
import dev.patrickgold.metroboard.lib.compose.MetroboardUnsavedChangesDialog
import dev.patrickgold.metroboard.lib.compose.Validation
import dev.patrickgold.metroboard.lib.ext.Extension
import dev.patrickgold.metroboard.lib.ext.ExtensionComponent
import dev.patrickgold.metroboard.lib.ext.ExtensionComponentName
import dev.patrickgold.metroboard.lib.ext.ExtensionDefaults
import dev.patrickgold.metroboard.lib.ext.ExtensionEditor
import dev.patrickgold.metroboard.lib.ext.ExtensionJsonConfig
import dev.patrickgold.metroboard.lib.ext.ExtensionMaintainer
import dev.patrickgold.metroboard.lib.ext.ExtensionManager
import dev.patrickgold.metroboard.lib.ext.ExtensionMeta
import dev.patrickgold.metroboard.lib.ext.ExtensionValidation
import dev.patrickgold.metroboard.lib.ext.validate
import dev.patrickgold.metroboard.lib.io.MetroboardRef
import dev.patrickgold.metroboard.lib.io.ZipUtils
...
import java.util.*
import org.metroboard.lib.compose.MetroboardButtonBar
...
import org.metroboard.lib.kotlin.io.writeJson
...
private fun EditScreen(
    ...
    val extEditor = workspace.editor ?: return@MetroboardScreen
    ...
    fun handleSave() {
        ...
        val sourceRef = if (isCreateExt) {
            MetroboardRef.internal(ExtensionManager.IME_THEME_PATH).subRef(flexArchiveName)
        } else {
    ...
    content {
        ...
        if (showUnsavedChangesDialog) {
            MetroboardUnsavedChangesDialog(
                onSave = {
                    handleSave()
                },
                onDiscard = {
...
private fun ManageMetaDataScreen(
    ...
    val meta = workspace.editor?.meta ?: return@MetroboardScreen
...
@Composable
private fun ManageDependenciesScreen(workspace: CacheManager.ExtEditorWorkspace<*>) = MetroboardScreen {
    title = stringRes(R.string.ext__editor__dependencies__title)

    val dependencyList = workspace.editor?.dependencies ?: return@MetroboardScreen
    ...
    content {
        ...
        MetroboardOutlinedBox(
            ...
            MetroRadioButton(
                ...
                label = stringRes(R.string.ext__editor__create_component__from_existing),
            )
            MetroRadioButton(
                onClick = { createFrom = CreateFrom.EMPTY },
                selected = createFrom == CreateFrom.EMPTY,
                label = stringRes(R.string.ext__editor__create_component__from_empty),
            )
        }
            ...
            MetroboardOutlinedBox(
                ...
                for ((componentName, component) in components) {
                    MetroRadioButton(
                        ...
                        subLabel = componentName.toString(),
                    )
                }
            }
