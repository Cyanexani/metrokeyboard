...
import dev.patrickgold.metroboard.app.settings.theme.ColorPreferenceSerializer
import dev.patrickgold.metroboard.app.settings.theme.DisplayKbdAfterDialogs
import dev.patrickgold.metroboard.app.settings.theme.SnyggLevel
import dev.patrickgold.metroboard.app.setup.NotificationPermissionState
import dev.patrickgold.metroboard.ime.clipboard.CLIPBOARD_HISTORY_NUM_GRID_COLUMNS_AUTO
import dev.patrickgold.metroboard.ime.clipboard.ClipboardSyncBehavior
...
import dev.patrickgold.metroboard.ime.media.emoji.EmojiHairStyle
import dev.patrickgold.metroboard.ime.media.emoji.EmojiHistory
import dev.patrickgold.metroboard.ime.media.emoji.EmojiSkinTone
import dev.patrickgold.metroboard.ime.media.emoji.EmojiSuggestionType
...
import dev.patrickgold.metroboard.ime.smartbar.quickaction.QuickAction
import dev.patrickgold.metroboard.ime.smartbar.quickaction.QuickActionArrangement
import dev.patrickgold.metroboard.ime.smartbar.quickaction.QuickActionJsonConfig
import dev.patrickgold.metroboard.ime.text.gestures.SwipeAction
import dev.patrickgold.metroboard.ime.text.key.KeyCode
import dev.patrickgold.metroboard.ime.text.key.KeyHintConfiguration
...
import dev.patrickgold.metroboard.ime.theme.ThemeMode
import dev.patrickgold.metroboard.ime.theme.extCoreTheme
import dev.patrickgold.metroboard.ime.window.ImeWindowConfig
import dev.patrickgold.metroboard.lib.ext.ExtensionComponentName
import dev.patrickgold.metroboard.lib.util.VersionName
...
import org.metroboard.lib.android.isOrientationPortrait

val MetroboardPreferenceStore = jetprefDataStoreOf(MetroboardPreferenceModel::class)

@Preferences
abstract class MetroboardPreferenceModel : PreferenceModel() {
    companion object {
        const val NAME = "metroboard-app-prefs"
    }
    ...
    inner class Clipboard {
        ...
        val syncToMetroboard = enum(
            key = "clipboard__sync_to_metroboard",
    ...
    inner class Dictionary {
        ...
        val enableMetroboardUserDictionary = boolean(
            key = "suggestion__enable_metroboard_user_dictionary",
            default = true,
        )
    }
        ...
        return when (entry.key) {
            ...
            "media__emoji_recently_used" -> {
                ...
                val recent = emojiValues.map {
                    dev.patrickgold.metroboard.ime.media.emoji.Emoji(it, "", emptyList())
                }
            ...
            // Keep migration rules until: 0.7 dev cycle
            "clipboard__sync_to_metroboard", "clipboard__sync_to_system" -> {
                entry.transform(
