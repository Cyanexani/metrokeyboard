...
import dev.patrickgold.metroboard.ime.dictionary.FREQUENCY_MAX
import dev.patrickgold.metroboard.ime.dictionary.FREQUENCY_MIN
import dev.patrickgold.metroboard.ime.dictionary.UserDictionaryDao
import dev.patrickgold.metroboard.ime.dictionary.UserDictionaryEntry
import dev.patrickgold.metroboard.ime.dictionary.UserDictionaryValidation
...
private val AllLanguagesLocale = MetroboardLocale.from(language = "zz")
...
enum class UserDictionaryType(val id: String) {
    METROBOARD("metroboard"),
    SYSTEM("system");
}

@Composable
fun UserDictionaryScreen(type: UserDictionaryType) = MetroboardScreen {
    title = stringRes(when (type) {
        UserDictionaryType.METROBOARD -> R.string.settings__udm__title_metroboard
    ...
    var currentLocale by remember { mutableStateOf<MetroboardLocale?>(null) }
    var languageList by remember { mutableStateOf(emptyList<MetroboardLocale>()) }
    ...
    fun userDictionaryDao(): UserDictionaryDao? {
        return when (type) {
            UserDictionaryType.METROBOARD -> dictionaryManager.metroboardUserDictionaryDao()
            UserDictionaryType.SYSTEM -> dictionaryManager.systemUserDictionaryDao()
        }
    }

    fun getDisplayNameForLocale(locale: MetroboardLocale): String {
    ...
    val importDictionary = rememberLauncherForActivityResult(
        ...
        onResult = { uri ->
            ...
            val db = when (type) {
                UserDictionaryType.METROBOARD -> dictionaryManager.metroboardUserDictionaryDatabase()
    ...
    floatingActionButton {
        MetroButtonItem(
            text = "+ " + stringRes(R.string.settings__udm__dialog__title_add),
                    ...
                    if (isInvalid) {
                        ...
                        val entry = UserDictionaryEntry(
                            ...
                            locale = locale.trim().takeIf { it.isNotBlank() }?.let {
                                MetroboardLocale.fromTag(it).localeTag()
                            },
                        )
                        if (isAddWord) {
