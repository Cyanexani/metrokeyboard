...
import dev.patrickgold.metroboard.lib.io.FileRegistry
...
import org.metroboard.lib.android.writeFromFile
...
import org.metroboard.lib.compose.MetroTriStateCheckboxPreferenceItem
...
@Composable
fun BackupScreen() = MetroboardScreen {
    ...
    suspend fun prepareBackupWorkspace() {
        ...
        if (backupFilesSelector.jetprefDatastore) {
            val fileBasedStorage = workspace.inputDir
                ...
                .subFile("${MetroboardPreferenceModel.NAME}.${AndroidAppDataStorage.JETPREF_FILE_EXT}")
                .let { FileBasedStorage(it.path) }
            MetroboardPreferenceStore.export(fileBasedStorage).getOrThrow()
        }
    ...
    content {
        Column(modifier = Modifier.fillMaxWidth()) {
            MetroHeader(title = stringRes(R.string.backup_and_restore__back_up__destination))
            MetroRadioButton(
                onClick = {
                    backupDestination = Backup.Destination.FILE_SYS
                },
                selected = backupDestination == Backup.Destination.FILE_SYS,
                label = stringRes(R.string.backup_and_restore__back_up__destination_file_sys),
            )
            MetroRadioButton(
                onClick = {
                ...
                label = stringRes(R.string.backup_and_restore__back_up__destination_share_intent),
            )
        }
        BackupFilesSelector(
...
internal fun CheckboxListItem(
    ...
    MetroCheckboxPreferenceItem(
        ...
        modifier = Modifier.padding(start = if (isSecondaryListItem) 28.dp else 0.dp),
    )
}
...
internal fun TriStateCheckboxListItem(
    ...
    MetroTriStateCheckboxPreferenceItem(
        title = text,
        state = state,
        onClick = onClick,
        modifier = Modifier.padding(start = if (isSecondaryListItem) 28.dp else 0.dp),
